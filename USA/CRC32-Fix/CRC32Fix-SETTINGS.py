...
for i in glob.iglob('../build/UNOSetting.sav', recursive=True):
    if os.path.getsize(i) == 0x2618:	#size of the overall UNOSetting.sav file
        with open(i, 'rb+') as f:
            ...
            if data[0x160:0x168] == b'\xcb\x35\x9c\x34\xcb\x35\x9c\x34':	#sanity-check for a specific string to be found in the save (specifically offset 0x160) ; string is 8 bytes long
